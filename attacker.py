...
def perturb_collusion(grads_list, update_list, malicious_users, root_grads, args):
    ...
    if args.attack_type == "alie":
        ...
        for idx in range(len(grads_list[0])):
            bad_gradient = gradient_mean[idx] - gradient_std[idx] * z_max * args.alie_scale
        ...
        for idx in range(len(grads_list[0])):
            bad_gradient = m[idx]
            bad_update = -args.lr * m[idx]

            for user_idx in malicious_users:
                grads_list[user_idx][idx].copy_(m[idx])
                update_list[user_idx][idx].copy_(-args.lr * m[idx])
